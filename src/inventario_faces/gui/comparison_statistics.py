...
def _summary_popup_html(
    ...
    *,
    ...
    return (
        ...
        "</p>"
        "<p>"
        f"Inventário por indivíduo (consolidação de repetições dentro de cada grupo): "
        f"o Padrão reúne <b>{summary.set_a_individuals}</b> indivíduo(s) distinto(s) "
        f"(<b>{summary.set_a_repeated_individuals}</b> com repetição) e o Questionado, "
        f"<b>{summary.set_b_individuals}</b> (<b>{summary.set_b_repeated_individuals}</b> com repetição). "
        "O agrupamento por indivíduo é uma camada informativa; as similaridades e a razão de "
        "verossimilhança continuam sendo calculadas par-a-par entre as faces selecionadas."
        "</p>"
        "<p>"
        f"O nível de significância adotado nesta leitura foi de <b>{significance_percent:.2f}%</b>; "

...
class FaceSearchWorker(QObject):
    progress_changed = Signal(int, str)
    log_message = Signal(str)
    completed = Signal(str)
    failed = Signal(str)

    def __init__(self, service: InventoryService, root_directory: Path, query_image_path: Path) -> None:
        super().__init__()
        self._service = service
        self._root_directory = root_directory
    ...
    def run(self) -> None:
        try:
            result = self._service.run_face_search(
                self._root_directory,
